...
ZIPNAME="perf-ginkgo-$(TZ=Asia/Kolkata date +"%Y%m%d-%H%M").zip"
ZIPNAME_KSU="perf-ginkgo-KSU-$(TZ=Asia/Kolkata date +"%Y%m%d-%H%M").zip"
...
sed -i 's/CONFIG_LOCALVERSION="-perf"/CONFIG_LOCALVERSION="-perf-KSU"/g' arch/arm64/configs/vendor/ginkgo-perf_defconfig
else
